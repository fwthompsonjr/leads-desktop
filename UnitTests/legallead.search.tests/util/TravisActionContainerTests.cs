...
{
    public class TravisActionContainerTests
    {
        [Fact]
        public void InstanceContainsContainer()
        {
            ...
            {
                var item = ActionTravisContainer.GetContainer;
                Assert.NotNull(item);
            });
            Assert.Null(error);
        }
        ...
        public void InstanceGetFetchCollection()
        {
            ...
            {
                var item = ActionTravisContainer.GetContainer;
                var children = item.GetAllInstances<ITravisSearchAction>();
        ...
        [Fact]
        public void InstanceGetFetchAlternateCollection()
        {
            var error = Record.Exception(() =>
            {
                var item = ActionTravisContainer.GetNonJusticeContainer;
                var children = item.GetAllInstances<ITravisSearchAction>();
                Assert.NotNull(children);
                Assert.Equal(7, children.Count());
            });
