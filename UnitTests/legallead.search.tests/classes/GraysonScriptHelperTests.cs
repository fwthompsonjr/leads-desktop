...
{
    public class GraysonScriptHelperTests
    {
        [Fact]
        ...
        {
            var testUri = GraysonScriptHelper.GetNavigationUri;
            Assert.True(Uri.TryCreate(testUri, UriKind.Absolute, out var _));
        }

        private static readonly Dictionary<string, string> collection = GraysonScriptHelper.ScriptCollection;
    }
}