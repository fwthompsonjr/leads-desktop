...
{
    internal class GraysonUiInteractive : BaseUiInteractive
    {
        public GraysonUiInteractive(WebNavigationParameter parameters) : base(parameters)
        {
            var container = ActionGraysonContainer.GetContainer;
        ...
        {
            const string countyName = "Grayson";
            ...
            result.Result = GenerateExcelFile(countyName.ToUpper(), 130);
        ...
        {
            return GraysonCourtLookupService.GetAddress(courtType, court);
        }
        ...
        private bool IterateCommonActions(bool isCaptchaNeeded, IWebDriver driver, DallasSearchProcess parameters, List<ICountySearchAction> common, ICountySearchAction a)
        {
            ...
            if (a is GraysonFetchCaseList _ && response is string cases) Items.AddRange(GetData(cases));
