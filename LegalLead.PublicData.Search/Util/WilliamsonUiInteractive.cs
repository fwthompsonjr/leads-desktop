...
{
    internal class WilliamsonUiInteractive : BaseUiInteractive
    {
        public WilliamsonUiInteractive(WebNavigationParameter parameters) : base(parameters)
        {
            var container = ActionWilliamsonContainer.GetContainer;
        ...
        {
            const string countyName = "Williamson";
            ...
            result.Result = GenerateExcelFile(countyName.ToUpper(), 120);
        ...
        {
            return WilliamsonCourtLookupService.GetAddress(courtType, court);
        }
        ...
        private bool IterateCommonActions(bool isCaptchaNeeded, IWebDriver driver, DallasSearchProcess parameters, List<ICountySearchAction> common, ICountySearchAction a)
        {
            ...
            if (a is WilliamsonFetchCaseList _ && response is string cases) Items.AddRange(GetData(cases));
